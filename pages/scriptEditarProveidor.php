...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">Procés editar proveidor</h1>
            </div>
            <!-- /.col-lg-12 -->
        </div>
        <!-- /.row -->
                        ...
                        switch(checkEsEmpresaById($id))
                        {
                            ...
                            case 1: //EMPRESA
                                ...
                                `persona_contacte2_client` = '$p2', `p2_email_client` = '$p2Mail', `comentari_client` = '$comentari', `dia_mensual_pagament_client` = '$diaMensualPagament',
                                 `numero_conta_client` = '$IBAN',`dies_fins_pagament_client` = '$diesFinsPagament', `forma_pagament_client` = '$formaPagament' WHERE `proveidors`.`id_client` = $id;";
                                insertClientBD($conn, $sql);
                                $errorComprovarEsEmpresa = false;
                                //echo $sql;
                                break;
                        ...
                        if (!$errorComprovarEsEmpresa)
                        {
                            ...
                            <div class=\"col-lg-12\">
                                <table style='margin-top: 20px;'>
                                    <tr>
                                        <td><form type='submit' action='mostrarProveidor.php' method='get'><input type=\"hidden\" name=\"id\" value=\"" . getProveidorIdFromNIF($nif) . "\"><button style='margin: 5px;' type='submit' class=\"btn btn-primary\"><i class=\"fa fa-arrow-left\"></i> Mostrar el proveidor</button></form></td>
                                        <td><form type='submit' action='mostrarProveidor.php' method='get'><input type=\"hidden\" name=\"mode\" value=\"1\"><input type=\"hidden\" name=\"id\" value=\"" . getProveidorIdFromNIF($nif) . "\"><button style='margin: 5px;' type='submit' class=\"btn btn-info\"><i class=\"fa fa-pencil\"></i> Torna a editar</button></form></td>
                                    </tr>
                                </table>
                                <br />

                            ";
                            mostrarProveidorTaula(getProveidorIdFromNIF($nif));
                            echo"</div>
                            <!-- /.col-lg-12 -->";
                        }
                        $conn->close();

                ...
                <div class="panel panel-default">
                    ...
                    <div  style="margin: 20px">
                        <table class="table table-striped table-bordered table-hover" id="clients1">
                            ...
                            $sql = "SELECT `id_client`, `nif_client`, `nom_client`, `cognom_client`, `poblacio_client`, `es_empresa_client`, `rao_social_client`, `nom_comercial_client` FROM `proveidors`";
                            $result = $conn->query($sql);
                            if ($result->num_rows > 0) {
                                // output data of each row
                                while($row = $result->fetch_assoc()) {
                                    if (!$row["es_empresa_client"]) {
                                        echo "<tr><td><a href='mostrarProveidor.php?id=" . $row["id_client"] . "'>" . $row["id_client"] . "</a></td><td>" . $row["nif_client"] . "</td><td><i class=\"fa fa-user\"></i> Particular</td><td>" . $row["cognom_client"] . "</td><td>" . $row["nom_client"] . "</td><td>" . $row["poblacio_client"] . "</td></tr>";
                                    }
                                    else
                                    {
                                        echo "<tr><td><a href='mostrarProveidor.php?id=" . $row["id_client"] . "'>" . $row["id_client"] . "</a></td><td>" . $row["nif_client"] . "</td><td><i class=\"fa fa-industry\"></i> Empresa</td><td>" . $row["rao_social_client"] . "</td><td>" . $row["nom_comercial_client"] . "</td><td>" . $row["poblacio_client"] . "</td></tr>";
                                    }
                                }
                            } else {
